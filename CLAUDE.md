...
## v2.1.5: 検索置換の再適用修正 / 文字詳細UI整理 / 保存導線更新

このバージョンでは、検索置換で一部文字だけにスタイルを当てる処理、再度全変換したときの設定維持、テキスト編集まわりのフローティングパネル、写植開始ダイアログ、保存メニュー、未保存終了確認をまとめて調整した。

### 検索置換 / 全変換

- 検索置換で置換文字列の一部だけに文字色などを反映できるよう、文字単位の色指定を保持して Photoshop 保存用 payload / JSX まで渡すようにした。
- 全変換後に置換文字や各項目を変更して再実行しても、文字色以外のサイズ・フォント・太字・斜体などが反映されるようにした。
- 再度全変換するときにチェック項目や入力値がリセットされないよう、ダイアログ状態を維持するようにした。
- 検索置換のフォント選択を検索対応コンボボックスにそろえた。
- 文字サイズ変更時に置換結果の改行が崩れにくいよう、プレビュー側の空白・折り返し処理を調整した。

### 文字スタイル / 文字詳細

- 基本スタイルのサイズ欄に、環境設定と同期する文字サイズ刻みトグルを追加した。刻みはサイズと横並びで、ラベルはトグルの上に表示する。
- 太字・斜体を文字詳細からサイズ欄の下へ移動し、Photoshop 風の `T` アイコンを追加した。
- 文字詳細の並びを `字間 → 長体平体 → 縦中横 → 全角半角` に統一し、サイドバーとフローティングパネルの順序をそろえた。
- 行間を editor-tab-panel の `+/-` から変更している最中にも、表示上の行間が即時更新されるようにした。複数行の一部選択時も対象行の見た目が動くように補正した。

### フローティングパネル / 表示切替

- `font-panel-floating layer-font-panel` がサイドバーに被らないよう、ルビパネルと同じ回避処理を入れた。
- `editor-tab-panel ruby-panel-floating` に `ルビ` / `文字詳細` タブを追加し、選択中テキストの文字詳細をフローティング側からも調整できるようにした。
- PDF/PSD 並列表示と見開き表示のトグルを縦型にし、side-toolbar のズーム上へ移動した。

### 写植開始ダイアログ / プロジェクト

- 「写植用ファイルを選択」ダイアログで、基本ポイント数と基本フォントを設定できるようにした。
- 基本フォントのドロップダウンがダイアログ下端で切れないよう上向きに展開し、右端の三角も上向きに変更した。
- プロジェクトカードから開くときの初期フォルダを `Script_Output/OPUSプロジェクト` にした。

### 保存 / 終了

- ヘッダーの「プロジェクトを開く」と単独の Photoshop 保存ボタンを削除した。
- プロジェクト保存ボタンを MojiQ 風のドロップダウンに変更し、`プロジェクト保存` / `PSD保存` / `両方保存` を選択できるようにした。
- 未保存の状態でウィンドウの閉じるボタンを押したときに警告ダイアログを表示するようにした。
- 「保存せずに終了」で閉じられないケースに備えて、ウィンドウ破棄とプロセス終了のフォールバックを追加した。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.1.5` に更新。
