...
## 2026-06-21 変更メモ: v2.6.9 リリース（**ドライブ式→ドライブ式**の自動更新確認用・機能変更なし）

ドライブ配信版（v2.6.8）の端末が、**次のドライブ配信版へ自動更新できるか**を検証するための版上げ（機能変更なし）。`App_installer\OPUS\` に `OPUS_2.6.9_x64-setup.exe`(+`.sig`) を配置（本体exe未署名＋インストーラのみ Authenticode 署名＋OPUS鍵で`.sig`再生成＋minisign VERIFY OK）。GitHub タグは打たない（ドライブ式）。v2.6.8 → v2.6.9 がドライブ→ドライブ更新の確認パス。
...
- `Ctrl+X` で選択中テキストレイヤーをカットし、内部クリップボードへ保持するようにした。既存レイヤーは保存 payload で削除扱いにし、新規レイヤーは状態から除去する。
- `Ctrl+V` で別ページにも同じ PSD 座標へペーストできるようにした。貼り付け先ページでは新規テキストレイヤーとして生成する。
- 別ページからペーストしたテキストは、原稿テキスト本文へ追記し、`sourceTxtRef` を付けてテキストエディタと配置済みレイヤーの同期対象にした。
- 削除済み扱いの既存レイヤーは overlay / 選択 / テキストエディタの一覧から除外するようにした。

### 保存まわり

- テキストレイヤーが入っていない PSD でも、読み込み済みページとして保存 payload に含めるようにした。
- 保存ボタン / ショートカットのガードを「編集差分あり」ではなく「PSD 読み込み済み」に寄せ、差分がない PSD も Photoshop 側へ出力できるようにした。
- 既存テキストレイヤー削除を JSX payload に含め、保存時に Photoshop 側で対象レイヤーを非表示化するようにした。

### OPUS プロジェクトファイルの多重起動連携

- `tauri-plugin-single-instance` のイベントで、OPUS 起動中に Explorer から `.opus` プロジェクトファイルを開いた場合でも、既存ウィンドウへ引数を渡してプロジェクトを開けるようにした。
- 2 回目起動時は既存ウィンドウを表示・フォーカスし、`second-instance-args` イベントで正規化済み launch args をフロントへ通知する。

### テキストエディタの段落ドラッグ移動

- `.editor-page-paragraph` 右側にドラッグ用グリップを追加し、pointer drag で段落を並べ替えられるようにした。WebView 上で HTML5 drag/drop が開始しない問題を避けるため、`pointerdown` / `pointermove` / `pointerup` で独自実装している。
- 全ページ表示ではページをまたいだ段落移動をサポートした。移動先段落の上半分 / 下半分で挿入位置を判定し、空ページにもドロップできる。
- 段落移動時は原稿本文の `<<NPage>>` セクションを差し替え、同時に `sourceTxtRef.pageNumber` / `paragraphIndex` を補正する。
- ページをまたぐ移動では、該当段落を参照している新規テキストレイヤーの `psdPath` も移動先ページの PSD path へ差し替える。座標やスタイルは維持し、PSD 側でも同じ位置にテキストが移動する。
...
`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.4.5` に更新。
...
- **フォント検索コンボの共通化**: `src/font-combobox.js` を追加し、表示名/PostScript名/aliases の NFKC 正規化検索、上下キー/Enter/Escape、候補 highlight、blur 復元、外側クリック close、候補クリック時の `mousedown.preventDefault()` を共通化した。サイドバー、検索置換の変換元/変換先フォント、写植再利用のフォント指定、ホーム写植設定が同じ基盤を使う。
- **写植再利用フォント指定の入力修正**: `pickReuseFontSize` のフォント欄を `<select>` から検索可能なテキスト入力へ変更し、初期表示ではリストを開かず、フォント欄クリックで caret が入り、入力で候補を絞れるようにした。リスト項目の hover/highlight が入力欄クリックを奪わないようにし、サイドバーのフォント検索と同じ操作感に合わせた。
- **既存フォント UI の挙動維持**: サイドバーのフォント候補プレビューは従来どおり IntersectionObserver で可視候補だけ `ensureFontLoaded` し、重い全件ロードを避ける。検索置換では hidden input と有効化 checkbox の更新、ホーム写植設定では表示名と PostScript 名の解決を維持した。
- **パス処理の小整理**: TXT 読込、エディタ保存名、スキャン抽出、自動配置でローカルに重複していた単純な `baseName` 実装を `src/utils/path.js` の `baseName` へ寄せた。UI 固有の複数引数 join や親フォルダ処理はそのまま残し、挙動変更を避けた。
- **RDD 更新**: `RDD.md` にフォント検索コンボの要件（入力欄優先、検索対象、キーボード操作、blur/click処理、フォントプレビュー）とパス処理ユーティリティの要件を追記し、push 前チェックリストにも確認項目を追加した。
- **ホーム画面のカード位置調整**: `home-divider` と `home-start-grid` の位置関係を CSS 変数化し、ウインドウサイズが変わっても divider からの間隔が一定になるようにした。ユーザー確認に合わせてカード列を少し上へ寄せ、desktop / mobile / 低高さ viewport で個別の間隔値を持たせた。
...
`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.4.3` に更新。
...
- サイドパネルのフチ色トグル右側に、現在のフチ仕様へ変換する丸矢印ボタンを追加した。変換対象は「すでにフチが付いているテキスト」のみに限定し、現在選択中のフチ色と太さを反映する。現在のフチが「なし」の場合は、既存フチ付きテキストのフチを外す。
- 検索・置換のヘッダーアイコンを差し替え、既存の全変換を「文字変換」に改名した。さらに「フォント変換」タブを追加し、検索文字なしで、指定した変換元フォントを使っているテキストにだけサイズ・フォント・色・フチ・長体/平体・カーニング/トラッキング・太字/斜体などを適用できるようにした。
- View メニューの「フォント帳 / PSD」の下に「ビューアー / PSD」を追加した。左パネルで A/B の 2 ファイルを扱い、A は現在読み込んでいる見本画像、B は手動で読み込んだ JPG / PNG / PDF / PSD を表示する。A/B トグルで交互に比較でき、B 読込後は「Bを更新」へ文言を変える。
- 左ビューアーの toolbar / stage / A/B トグルを写植見本側の表示方法に合わせて調整した。画像表示は stage の実寸から表示サイズを再計算し、ウインドウ縮小時も余白を出さずに写植見本と同じスクロール表示にする。A ボタンは「Aを更新」に統一し、クリアボタンは削除した。
- PDF のページ・回転・単ページ化・先頭白紙除外・ズーム変更に合わせて A を同期する。見本範囲外や同期失敗時に古い A 画像が残らないよう、A 側を空に戻すガードも追加した。
...
`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.3.9` に更新。
...
- PSD 上のテキストを直接編集して空行を入れたとき、原稿テキスト側へ逆同期して段落が分割され、配置済みレイヤーが前半だけへ追従して文字欠けする問題を修正した。PSD 直接編集の空行は原稿段落区切りとして扱わず、同じ吹き出し内の空行として保持する。
- 改行処理の CR/LF 取り扱いを `\r\n` / `\r` / `\n` すべてに揃え、行数・行開始位置・縦書き bbox・Photoshop 書き出しで改行種別によるズレを抑えた。
- テストモードを通常作業画面に近づけ、A4 縦 72dpi 換算の白紙 PSD/PDF、`テスト原稿.txt`、`sourceTxtRef` 付き配置テキストを生成するようにした。テストモードの縦書きテキストは詰め枠で左余白を抑える。
- リサイクルの「フォント・サイズを指定」でも、指定フォント/サイズを基本値にしつつ、PSD/背景判定由来の中丸ゴシック切替、白フチ、検出サイズ、文字色を反映するようにした。
- リサイクルの中丸判定で、PSD テキストの細い bounds だけでなく拡張 bounds も解析し、ウニ吹き出し外周を拾いやすくした。スコアがしきい値以上なら bucket 0 でも中丸フォントへ切り替える。
- Photoshop 読み取り経路で `textItem.color` を取得し、黒/白/HEX 色をリサイクル配置の新規レイヤーへ渡すようにした。
- V ツールのホバー選択遅延を 0.7 秒から 0.4 秒に短縮した。
- 原稿テキスト入力欄の placeholder を `テキストを入力（Ctrl＋Enter）` に変更した。
...
`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.3.7` に更新。

## 2026-06-05 変更メモ: v2.3.6 リリース

v2.3.6 では、リサイクル進捗の見え方、文字サイズ単位切替時のサイズ操作、テキスト編集時の視認性、ホーム/ダイアログ UI を中心に調整した。

- リサイクル開始時の進捗表示を、通常のロード表示ではなく `PSD読み取り` → `テキスト抽出` → `配置調整` → `表示準備` の段階表示へ変更した。PSD 一括読み取り、テキスト抽出、原稿テキスト生成、見本ペイン準備、フォント読み込み、配置中心補正が進捗に反映される。
- OPUS から ProGen を開く探索先に `Desktop\ネイティブデータ\progen\src-tauri\target\release\progen.exe` を追加した。見つからない場合のエラーメッセージにも同パスを含める。
- 文字サイズ単位を `pt` 以外の `mm` / `級` に切り替えている場合も、`Ctrl + ↑/↓` のサイズ変更が現在の表示単位の刻みに沿って連続動作するようにした。単位変換の浮動小数誤差で最初の1回だけ動く状態を避けるため、単位ごとの許容誤差でスナップ判定する。
- 単位切替後の拡大縮小で縦書きテキストがフレーム内で見切れる問題を修正した。縦書きの bbox 厚み方向に最小限の安全幅を戻し、後置 auto-fit が既存の安全幅を縮めないようにした。
- テキストをダブルクリックして in-place 編集する際、テキストフレーム範囲に白 60% の半透明背景を表示し、黒背景上の黒文字でも読みやすくした。
- 白フチ付きテキストの編集中は、古い白フチ underlay を削除し、実際の編集テキスト自身へフチ preview を描くようにした。改行時に白フチだけ取り残される問題を防ぎつつ、テキスト選択中も白フチ表示を維持する。
- ホーム画面カードの説明文は、説明ブロック自体をカード中央に置きながら、文字は左揃えになるようにした。
- 「フォント・サイズの扱い」ダイアログの subtitle を中央揃えにし、`写植見本を再現` / `フォント・サイズを指定` のアイコン枠を固定して高さを揃えた。説明文の行数差で右側アイコンが上に寄らないよう、アイコン位置は上基準で固定した。
...
`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.3.6` に更新。

## 2026-06-04 変更メモ: v2.3.5 リリース

v2.3.5 では、リサイクル写植のサイズ・位置再現と、高信頼のデッドコード削除を中心に調整した。

- リサイクルで写植したテキストのサイズ変更時に、表示矩形が PSD 由来 bbox から OPUS 仕様 bbox へ切り替わり、上下位置がずれる問題を修正した。PSD 由来の source bounds を持つ再利用レイヤーは、サイズ変更後も元 bbox を比率スケールして使う。
- 14pt の PSD テキストをリサイクルした際、浮動小数の誤差で OPUS 上の表示が 13.9pt に縮小される問題を修正した。再現サイズは小数1桁で丸め、13.999... を 14.0pt として扱う。
- 高信頼のデッドコード候補を削除した。保存後レビュー用モーダル一式、旧保存フォルダ命名ヘルパー、未使用 export、未使用 CSS、未使用 npm 依存の `@tauri-apps/plugin-dialog` を整理した。
- JS 側の dialog plugin 依存は削除しつつ、Rust/Tauri 側の `tauri-plugin-dialog` は実使用中のため維持した。

### 検証

- `npm run check`
- `cargo check`
- `npx --yes knip --reporter compact` で高信頼削除対象が残っていないことを確認。中〜低信頼の unused export 候補は別判断対象として残した。

## 2026-06-04 変更メモ: v2.3.4 リリース

v2.3.4 では、写植再利用の再現精度、写植見本/PSD の表示倍率一致、原稿テキスト連携、ダイアログ UI の統一を中心に調整した。

- ホーム画面の「リサイクル」カード文言を「既存PSDのテキストを / 再編集」に整理し、カード説明を左揃えにした。
- 「フォント・サイズの扱い」ダイアログを写植用ファイル選択ダイアログと同じカード型レイアウトに変更した。文言は「写植見本を再現」「フォント・サイズを指定」に改め、見本再現側のアイコンも重なりのきれいな書類アイコンへ差し替えた。
- 写植見本を再現するリサイクル時に、元テキストレイヤーの `sizePt` を優先して使うようにし、サイズの小数点第2位以下を切り捨てる処理を追加した。
- 縦書き再利用テキストの bbox 厚み計算を修正し、1行目に余分な行送りを掛けないようにした。これにより左側余白や左端の削れが起きにくくなった。
- 再利用で生成したテキストに元レイヤーの bounds 情報を持たせ、内容とサイズが元のままのときは写植見本側の矩形を優先して表示するようにした。
- リサイクルで抽出したテキストを原稿テキストとテキストエディタにも反映するようにした。ページごとに `<<NPage>>` ブロックを作り、生成レイヤーには `sourceTxtRef` を付与する。
- サイズ単位を変更したとき、レイヤーバッジなどテキストプロパティ上のサイズ表示も選択中の単位へ追従するようにした。
- 写植見本/PSD 編集モードでは、見本側の表示サイズをPSD現在ページの実表示サイズに合わせるようにした。左右ペインの幅差やスクロールバー差による微妙な倍率ズレを避ける。
- すべてのキャンセルボタンの形状と hover 色を、写植用ファイル選択ダイアログのキャンセルボタンに統一した。
...
## 2026-06-04 変更メモ: v2.3.3 リリース

v2.3.3 では、原稿テキストからの配置操作、検索/置換の一括変換、写植見本/PSD編集モードへの遷移制限、フォント登録まわりを中心に、実作業中に引っかかる細部をまとめて調整した。

- ホーム画面から「書き起こし」を選択した場合は、View メニューの写植見本/PSD など PSD 編集モードへ移る項目を disabled 扱いにしてグレーアウトした。テキスト抽出作業中に誤って PSD 編集モードへ移動しないようにし、`psd-stage` の空状態アイコンと案内メッセージも削除した。
- テキストエディタのルビボタンを削除し、原稿テキスト/テキストエディタともに Ctrl+Enter ですぐ配置できるようにした。入力欄からフォーカスが外れていても、最後に入力した内容を Ctrl+Enter で配置できるようにした。
- 検索/置換をヘッダーの View 右隣へ移動し、虫眼鏡+チェック形のアイコンに変更した。レンズと持ち手の余計な突起をなくし、ヘッダー内で他の操作と並ぶ導線にした。
- サイズ単位ボタンに薄い枠線を付け、クリックで `pt` / `級` / `mm` を選べるドロップダウンにした。ドロップダウン表示も `級` に統一した。
- 検索/置換の全変換に、白フチサイズ/フチ色を一括変換できる項目を追加した。さらに、全変換タブでは検索文字が空でも、チェックを入れた変換項目を対象レイヤー全体へ適用できるようにした。
- AR/DF など一部フォントが WebView の `FontFace` 登録で失敗するケースに対して、Rust 側でフォントファイルの読み出しを補助し、フロント側で複数候補の登録を試せるようにした。フォントブック/フォントローダー側も、登録済み family/postscript 名の扱いを調整した。
...
## 2026-06-03 変更メモ: v2.3.2 リリース

v2.3.2 では、テキスト配置・縦中横・保存状態・テキストエディタ連携まわりの実機フィードバックを中心に修正した。

- 横長の記号テキストや縦中横テキストで、表示上の BOX と PSD 出力時の座標がずれやすい問題を調整した。特に `!?` などの縦中横記号は、縦書き行に対して左寄り・右寄りになりすぎないよう中央寄せを見直した。
- 縦書き時の半角から全角への変換を見直し、単独の `?` / `!` が半角扱いのまま縦中横化されて横倒しになる問題を修正した。
- 原稿テキストをエディタ上で改行したとき、改行後の段落だけを新しいテキストとして中央配置し、既存配置済みテキストの位置や後続テキストが巻き添えでずれないようにした。
- PSD 保存後に Adobe Crash Processor の確認が残るケースへ対応し、保存後に不要な確認プロセスを解除する処理を追加した。
- PSD とプロジェクトを保存済みでもウィンドウを閉じる際に未保存警告が出る問題を修正した。
- 見開きモードでは、1ページ目以降のページ送りを見開き前提で 2 ページずつ進めるようにした。
- テキストエディタの正誤チェック・提案チェックのリンクを、該当ページへ遷移できるよう修正した。
- テキスト保存先フォルダ名を `OPUSテキスト` に変更した。
- 写植用ファイル選択で PSD 確認中も、見本ファイルと同じようにカード内プログレスバーを表示するようにした。
- 保存したテキストの `<<NPage>>` マーカー直後に、1 行分の空行を入れるようにした。
- `editor-page-paragraph` の長押しドラッグ並び替えは、移動不能になる不具合が続いたため関連コードを削除した。PSD に配置済みのテキストへ影響する同期抑止コードも合わせて戻した。
...
## 2026-06-03 変更メモ: v2.3.1 リリース

v2.3.1 では、ホーム画面・基本スタイルの単位切替・サイズ調整ショートカット・テキスト選択 UX・Photoshop/ProGen 連携の安定化をまとめて反映した。

- ホーム画面の `home-start-grid` を横一列配置にし、カード位置を区切り線から下げて、バージョン表記との余白バランスに合わせた。「写植再利用」はユーザー向け文言を「リサイクル」に変更した。
- 基本スタイルタブの文字サイズ単位は、単位表示部をクリックして `pt` / `Q` / `mm` を循環切替できるようにした。内部値は pt を維持し、表示と入力だけを選択中単位へ変換する。
- サイズ調整時の `Ctrl + ↑/↓` は、環境設定で指定した刻みサイズに連動するようにした。±ボタンや他のサイズ操作と同じステップを参照する。
- テキストレイヤー上で 0.7 秒ホバーすると、クリックなしでも対象レイヤーが選択状態になるようにした。移動や編集に入る前の選択補助として動作する。
- PSD 保存時の Photoshop 連携を調整した。OPUS が起動した Photoshop セッションだけを保存後に終了し、既に開いていた Photoshop は閉じずに最小化復帰する。処理中は Photoshop 本体ウィンドウと COMIPA などのプラグインパネルを継続的に隠し、完了直後の一瞬表示も抑える。
- Photoshop のスクラッチディスク減少について調査し、OPUS 側の `psdesign*` 一時ファイルではなく Photoshop 側の `Photoshop Temp*` が大半を占めることを確認した。OPUS 起動セッションでは JSX 終了時に Photoshop キャッシュ purge と `app.quit()` を行い、テンポラリ肥大化を抑制する。
- ProGen 起動時は `CREATE_NO_WINDOW` を付け、`.bat` / `.cmd` / `.lnk` / `.exe` の各起動経路でターミナルウィンドウが出ないようにした。
- OPUS から ProGen へ渡すハンドオフを、直接校正画面へ送る `proofreading` モードから `home` モードへ変更した。ProGen はホーム画面から開始し、ジャンルとレーベル選択後に OPUS で保存したテキストを読み込む流れを想定する。
...
## 2026-06-01 変更メモ: v2.2.9 リリース

v2.2.9 では、ホーム画面の進捗表示順、デスクトップショートカット経由のファイル読込、PSD 読込前チェック、100MB 以上の見本 PDF の扱い、Tauri パッケージ整合性を中心に修正した。

- 書き起こし・写植自動配置・プロジェクト読込の進捗タスク表示を整理し、通常の写植自動配置と同じく上から下へ進む表示に揃えた。書き起こしフローでも `reference-load` → `scan` → `transcribe` → `editor-ready` の段階が分かる。
- デスクトップショートカットから `.opus` / `.psd` / `.txt` / 見本ファイルを読み込めるようにした。起動引数・ファイルピッカー・フォルダ列挙で `.lnk` の実体を解決し、PowerShell/Photoshop 連携などの外部プロセス起動ではプロンプトウィンドウが出ないようにした。
- PSD 選択時にモノクロ 2 階調 PSD を検出した場合、警告ダイアログを表示し、該当 PSD が含まれる間は開始ボタンをロックするようにした。
- 100MB 以上の見本 PDF は圧縮せず、警告ダイアログを出して読み込み対象から除外する仕様に変更した。圧縮処理・圧縮用進捗ダイアログ・圧縮 PDF 保存コマンド・`pdf-lib` 依存は削除した。
- `path_info` がファイルサイズ `sizeBytes` を返すようにし、見本 PDF のサイズ判定を実ファイル読込前に行えるようにした。
- `@tauri-apps/api` を `2.10.1` に固定し、Rust 側 `tauri v2.10.x` と npm 側 Tauri API の minor mismatch が起きないようにした。
...
## 2026-06-01 変更メモ: v2.2.8 リリース

v2.2.8 では、フォント帳表示、PSD 全画面表示、ファイル選択、ProGen 連携、PSD 保存時の外部プロセス起動、機械的チェックを中心に改善した。

- View メニューを「写植見本 / PSD」「校正チェック / PSD」「フォント帳 / PSD」「テキストエディタ」「PSD見開き」「PSD全画面モード」の構成に整理し、PDF ステージラベルバーから旧フォント帳ボタンを削除した。
- フォント帳を View の独立モードとして開けるようにし、フォント帳カードではサンプル画像を初期表示できるよう lazy-load の画像表示枠を追加した。
- PSD 全画面モードに閉じるボタンを常時表示し、ワークスペース反転時はサイドバーと反対側へ配置されるようにした。
- ファイル選択でデスクトップを開いたときにショートカット解決が連続実行され、ウィンドウが何度も出る問題を抑止した。ディレクトリ一覧では子要素の `.lnk` を一括解決せず、明示的に移動するパスのみ解決する。
- Windows の PowerShell ショートカット解決、Photoshop 保存起動、保存後の `taskkill` に `CREATE_NO_WINDOW` を付け、保存やファイル選択でターミナルウィンドウが表示されないようにした。
- テキスト保存後に ProGen を開く連携で、従来の handoff ファイルに加えて proofreading モード情報と JSON payload を書き出し、ProGen 側が校正プロンプトへ直接遷移できるようにした。
- PDF 見開き分割の仮想ページ生成を調整し、先頭白紙除外と横長見開き分割の組み合わせでページ順が崩れないようにした。
- テストモード完了時に `runMechanicalChecks` を実行する仕組みを追加し、PSD/PDF ページ状態、仮想ページ、同期/非同期ページ移動、ズーム状態、保存 payload の数値安全性を自動確認できるようにした。
...
## 2026-06-01 変更メモ: v2.2.7 リリース

v2.2.7 では、PSD 読み込み、ルビ編集、フォント検索、文字サイズ UI を中心に現場操作の引っかかりを解消した。

- PSD / テキスト / PDF のファイル選択で、デスクトップショートカットやショートカット先フォルダを解決できるようにした。Rust 側のパス情報・PSD 列挙・ディレクトリ列挙で `.lnk` の実体を追跡し、フロント側の拡張子判定も実パスを見られるようにした。
- テキストのダブルクリック編集時に、選択テキストを 2 倍表示できるようにした。環境設定の写植設定から適用 / 非適用を切り替えられる。
- ルビ編集を改善した。テキストエディタ側のルビ入力は「モノ / グループ / 自動」を選択可能にし、ルビありテキストのテキストプロパティ右側に「ルビ削除」ボタンを表示するようにした。
- ルビを直接クリックしたとき、親テキスト全体の巨大な編集フレームに入らず、該当ルビだけを選択してルビ編集パネルにフォーカスするようにした。テキスト選択を解除したときは、残ったルビ選択状態も破棄してルビパネルを閉じる。
- フォント検索でフォントを適用したあと、検索リストを再表示せず、入力欄に確定したフォント名だけを表示するようにした。
- 文字サイズの刻み UI をドロップダウン化し、`1` を追加した。刻み設定は環境設定にも反映される。
- サイドパネルの `panel-header` 直下に、文字サイズ単位を「ポイント / 級」で切り替える 2 列ボタンを追加した。内部値は pt のまま保持し、入力欄・単位表示・レイヤー一覧の表示だけを選択単位へ変換する。
...
- ルビありレイヤーでは、行ごとの行間を `textStyleRange` の固定 `leading` ではなく、`paragraphStyleRange.autoLeadingPercentage` へ反映するように戻した。
- `applyLineLeadingPercentages(...)` を追加し、`lineLeadings` の各行パーセントを同じ値ごとにまとめて `applyRubyAutoLeadingPercentage(...)` へ渡すようにした。
- 既存レイヤー・新規レイヤーの両方で、ルビあり時は段落側の行送り、ルビなし時は従来どおり `applyLineLeadings(...)` の固定 leading を使う分岐にした。
- `applyRubyAutoLeadingPercentage(...)` の行範囲計算を見直し、`\r\n` / `\r` / `\n` の改行長を実際の文字数として扱うようにした。これにより paragraphStyleRange の境界が改行コード差でずれにくくなった。

### 通常ルビ1行目の重なり修正

- 原因は、1行目の通常ルビだけ「前行との中間」が存在しないにもかかわらず、ルビ幅を考慮せず中心位置だけを横へずらしていたこと。ルビ文字が長い場合、中心だけの移動では左端が親文字へ食い込んでいた。
- UI側は `.ruby-text-first-line` の通常ルビを `left: calc(100% + 0.08em)` へ変更し、親文字 wrap の外側に左端を置くようにした。親文字レイヤーの範囲は広げず、ルビだけを横に逃がす。
- Photoshop保存側も `createRubyLayer(...)` で1行目通常ルビを UI 実測値ではなく親文字 range の外側配置にし、`NORMAL_FIRST_LINE_RUBY_GAP_EM` の余白だけを挟んで重なりを避けるようにした。
- 1行目通常ルビは専用配置になるため、通常ルビ用の親寄せ nudge を二重に掛けないよう除外した。

### 親文字指定UI

- 親文字指定ダイアログ内の文字セルサイズを、親文字レイヤーに合わせた算出値の 1.2 倍にした。
- サイドバーの「親文字指定」ボタンも `min-height: 29px` / `font-size: 14px` / `padding: 0 10px` にし、従来より約 1.2 倍の見え方へ調整した。

### 苦戦した点・注意

- 行間の不具合は、プレビュー側の `lineLeadings[i - 1]` 修正だけでは解決しなかった。Photoshop保存では `textStyleRange.leading` と `paragraphStyleRange.autoLeadingPercentage` のどちらが効くかが重要で、ルビあり時は段落側を分割して更新しないと、文字パネル上の狙った行間にならない。
- v2.1.4 のコメントには「ルビあり時は段落側で処理する」と書かれていたが、実装は固定 leading 経路へ寄っていた。今回、実装とコメントの食い違いも合わせて直した。
- 通常ルビ1行目の重なりは、`.ruby-text` の中心座標だけを見ていると見落としやすい。長いふりがなでは左端が親文字へ入るため、1行目だけは「中心をずらす」ではなく「ルビ左端を親文字外へ出す」考え方に変えた。

### 今回の主な検証

- `npm run lint` 成功。
- `npm run build` 成功。Vite の dynamic import / chunk size 警告は既存警告。
- `cargo check` 成功。
- ブラウザで `http://127.0.0.1:5176/` を開き、通常ルビ1行目の CSS ルールと「親文字指定」ボタンサイズの反映を確認。
- `npm run check:encoding` は、この作業フォルダが Git リポジトリではなく `git ls-files` を実行できないため未完了。コード変更によるビルド・lint・Rustチェックは通過。

---

## v2.1.4: ルビ編集の行消失修正 / 行間150%反映 / PSD保存位置補正

今回の版では、複数行テキストに対してルビを付けたときの編集画面と Photoshop 保存結果のずれを修正した。特に「一行目にルビを振ると二行目が消える」「二行目以降のルビで行間150%が効かない」「PSD保存時に一行目ルビや圏点が親文字から離れる」症状をまとめて調整している。

### ルビ編集中のテキスト保持

- contenteditable 内の文字位置変換を見直し、`.ruby-text` の文字を本文として数えないまま、`div` 行ブロック由来の仮想改行も正しく扱うようにした。
- ルビ表示だけを理由に本文を行ごとの `div` へ分割しないようにし、縦書きで後続行・後続列が消える経路を避けた。
- ルビ適用後は、行間変更の有無に関係なく編集中プレビューを再描画し、DOM と state のルビ情報を同期するようにした。

### 行間150%と矩形計算

- `lineLeadings[N]` は「行Nの leading」、つまり画面上では「行N+1 の前を開ける値」として扱うように統一した。
- プレビュー描画、既存レイヤー矩形、新規レイヤー矩形の参照を `lineLeadings[i - 1]` に修正し、二行目以降にルビを入れたとき自動で 150% 分の余白が開くようにした。
- Photoshop 保存用 JSX でも、ルビありレイヤーで `applyLineLeadings` をスキップしないようにし、`charRubies` から必要な行間 override を補完してから固定 leading を適用するようにした。

### PSD保存時のルビ・圏点位置

- 一行目のルビは「前行との中間」ではなく通常の親文字横位置を使うようにし、前行がない場合の仮想行間へ逃げる挙動を止めた。
- 圏点・中黒などの特殊ルビも保存前の実測対象に含め、画面上の実際の位置を `offsetX/Y` と `absX/Y` として Photoshop に渡すようにした。
- JSX 側でも特殊ルビだけ fallback 計算へ戻す分岐を外し、ビューアーで見えている位置を PSD 保存結果に反映する方針に揃えた。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.1.4` に更新。
...
## v2.1.3: ルビ編集強化 / 特殊ルビ移植 / Photoshop保存反映

この版では、`C:\Users\noguchi-kosei\Desktop\ネイティブデータ\PsDesign-main` に入っていたルビ関連の最新変更を移植した。通常ルビに加え、親文字指定、濁点・中黒の特殊ルビ、ルビ文字そのもののサイズ編集、Photoshop保存時の実レイヤー生成まで一連で反映している。

### ルビUI / 親文字指定

- ルビ編集パネルに「親文字指定」ボタンを追加し、テキスト選択とは別に任意の親文字範囲を指定できるようにした。
- 親文字指定ダイアログを追加し、文字セル選択と分割位置指定で複数の親文字範囲を組めるようにした。
- ルビパネルに濁点 `゛` と中黒 `・` の特殊ルビボタンを追加した。
- 特殊ルビボタンは選択範囲に同種のルビがある場合に active 表示し、中黒と小書き仮名の組み合わせでは警告状態を出すようにした。

### ルビ編集 / プレビュー

- ルビ文字を直接選択したとき、サイズ入力が親文字ではなくルビ側のスケールへ反映されるようにした。
- ルビ選択中は太字・斜体の親文字スタイル操作を無視し、意図しない親文字側の変更を防ぐようにした。
- 通常ルビに重ねる overlay ルビを `state.charRubies` に保持できるようにし、濁点・中黒を既存ルビの上に追加/削除できるようにした。
- インプレース編集中のルビDOMを更新し、通常ルビ、重ねルビ、モノルビグループ、先頭行の中黒補正が画面上で安定して見えるようにした。
- ルビ計測時に `.ruby-wrap` と `.ruby-mono-group` の両方を対象にし、overlay ルビの実表示位置もPSD座標として保存するようにした。

### Photoshop保存 / JSX

- Rust側の `RubyEntry` に `overlays` を追加し、重ねルビ情報をPhotoshop適用用JSXへ渡せるようにした。
- JSX生成で overlay ルビをシリアライズし、保存時に濁点・中黒も実テキストレイヤーとして生成するようにした。
- 既存の生成済みルビレイヤーを名前と親レイヤー近傍から掃除し、保存を繰り返しても重複ルビレイヤーが残りにくいようにした。
- 中黒・濁点の親文字マーク用オフセットを追加し、先頭行の中黒位置も補正した。
- overlay ルビの重複生成を抑止し、中黒は範囲重複でも二重に出ないようにした。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.1.3` に更新。
...
## v2.1.2: ホーム導線 / 見開き編集 / OPUS保存 / 検索置換 / ダイアログ整理

この版では、ホーム画面からの開始導線、右綴じ前提の見開き編集、OPUSプロジェクト保存、検索置換のスタイル適用、ダイアログの閉じ方を一連で整理した。ライトモードは廃止し、UIはダークモード前提に統一している。

### ホーム画面 / 開始導線

- ホーム画面に「プロジェクトを開く」を追加し、「書き起こし」「写植」と同じカード型の導線に揃えた。
- ヘッダー構成は変更せず、ホーム画面側のカード追加だけでプロジェクト読込へ入れるようにした。
- 写植用ファイル選択後、「開始」から位置調整選択へ進むまでのラグを軽減するため、見本ページ確認と位置調整ダイアログ表示の流れを整理した。

### 見開き編集 / 右綴じ表示

- `sync-segment` / `view-mode-segment` に、PSDを見開きで表示して編集できる「見開き編集」モードを追加した。
- 右綴じ前提で、1ページ目は右ページに本紙、左側に白紙が来る見開き構成にした。
- 最後のページも右綴じ順に合うよう、欠けるページ位置を補正した。
- ページが存在しない側は、濃い背景ではなく白紙ページとして表示するようにした。
- 見開きページ同士の間隔をなくし、左右ページが背でつながって見える表示にした。
- 定規とガイドを見開き両ページに対応させ、右ページ側にもガイドを引けるようにした。

### OPUSプロジェクト保存

- プロジェクト保存先を `Script_Output/OPUSプロジェクト`、写植出力先を `Script_Output/OPUS写植` に分離し、名称を用途別に整理した。
- OPUSプロジェクト内の既存フォルダへ保存する場合は、上書き確認ダイアログ「上書き保存してよろしいですか？」を表示するようにした。
- プロジェクトそのものの上書き保存に対応し、`.opus` のパス情報とテキスト変更を更新できるようにした。
- 上書き保存時にPSDフォルダや写植見本フォルダを再コピー・削除しない方針に変更し、PSD / 写植見本は保存時に変更を加えないようにした。
- ページ数を減らして再保存するケースでも、`.opus` とテキスト側の変更だけを上書き反映し、元PSD・見本ファイルは保持する。

### 検索置換 / 全変換

- 検索置換ダイアログで、サイドバーの基本スタイルと文字詳細を置換対象へ適用できるようにした。
- 「サイドバーのスタイルを適用」という一括チェックではなく、ダイアログ内に文字色、フチ、長体、平体、カーニング、トラッキング、太字、斜体などの項目を表示し、チェックした項目だけを置換結果へ反映する方式にした。
- 行間は今回の検索置換スタイル適用対象から除外した。
- ツールバーの検索置換ボタンアイコンを、虫眼鏡ではなく上下入れ替え矢印のアイコンへ差し替えた。

### ダイアログ / ボタン挙動

- 検索置換ダイアログの右上 `×` ボタンを削除し、右下のキャンセルボタンで閉じる仕様に統一した。
- 見本非表示選択、写植用ファイル選択、カスタムファイル選択など、右上 `×` を持っていたダイアログも同様に削除した。
- キャンセル系ボタンの hover を、写植用ファイル選択のキャンセルボタンと同じ赤背景 + 白文字の挙動に統一した。

### ライトモード廃止

- ライトモード切替を廃止し、関連するテーマ切替ボタン、`data-theme` / `psdesign_theme` 参照、ライトモード用CSSを削除した。
- 画面全体はダークモード前提の配色に一本化した。
...
## v2.1.1: テキスト/スタイル編集強化 / 字間保存 / 縦中横改善

この版では、サイドバーのテキスト編集とスタイル編集をPhotoshopの文字パネルに近い構成へ整理し、選択中テキストに対する長体・平体、カーニング、トラッキング、縦中横の編集を強化した。画面プレビューだけでなく、Photoshop経由のPSD保存にも反映されるようにし、縦書き時の半角数字と手動縦中横の扱いも調整している。

### テキスト編集 / スタイル編集

- 既存の「スタイル編集」を「テキスト編集」に改名し、新しい「スタイル編集」タブを追加した。
- 長体・平体をスタイル編集へ移動し、テキスト編集と同じ2列レイアウトで表示するようにした。
- テキスト編集とスタイル編集の項目にPhotoshop文字パネル風のSVGアイコンを追加し、スタイル編集の各 `style-control-label` には不要なアイコンを出さないようにした。
- スタイル編集の項目区切り線を、テキスト編集と同じくパネル間に1本だけ入る表示へ揃えた。

### 長体・平体 / 字間

- 選択文字に対する長体・平体の%指定を追加し、サイドバーの入力欄と±ボタンから連続入力・連続調整できるようにした。
- カーニングとトラッキングを追加し、カーソル位置または選択範囲の字間だけに適用されるようにした。
- サイドバー操作中にテキスト選択やカーソル位置が解除されないよう、インプレース編集の選択復元とハイライト維持を調整した。
- 長体・平体、カーニング、トラッキングをPSD保存ペイロードとJSX出力に含め、保存後のPhotoshop上にも反映されるようにした。

### PSD保存 / Photoshop連携

- 保存後にPhotoshopが開けなくなるケースを避けるため、Photoshop/Adobe Crash Processorまわりの終了処理を追加した。
- PSD保存後に確認用Photoshopを閉じてもAdobe Crash Processorが残る場合に、関連プロセスを終了できるようにした。

### 表示 / 操作性

- Alt+マウスホイールのズームを、画像中心ではなくポインター位置へ向かうPhotoshop風ズームに変更した。
- ウィンドウサイズ変更時にPDF側の表示位置がずれないよう、PDF/PSDペインのスクロール・リサイズ同期を調整した。

### 縦中横

- スタイル編集に「縦中横」項目を追加し、「適用」「解除」ボタンで手動指定できるようにした。
- 環境設定の「縦書き時の半角→全角自動変換」をサイドバーからも切り替えられるようにした。
- 自動縦中横は、縦書き中の独立した半角数字2桁だけに適用するようにした。1桁や3桁以上の数字は自動適用しない。
- 手動の縦中横ボタンは数字以外の文字にも適用できるようにし、改行をまたぐ選択では改行文字を除外して行ごとに適用するようにした。
- 縦中横のプレビューは文字サイズを維持し、横方向にはみ出しても見切れにくいよう `has-tcy` 表示を追加した。
- 手動縦中横の範囲をPSD保存時にも `baselineDirection=cross` として書き戻すようにした。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.1.1` に更新。

---

## v2.1.0: 保存アニメーション刷新 / 読込タスク表示 / 星座描画安定化

この版では、PDF/PSD読込とPhotoshop保存中のOPUS進捗表示を整理した。処理進捗と星座描画を無理に同期させず、処理内容は右下のタスク表示へ寄せ、保存時はPhotoshop保存専用の演出とページ表記に変更している。写植用ファイル選択中の見本読み込みでは、ページ数確認だけで大きな読込アニメーションが始まらないようにした。

### Photoshop保存アニメーション

- PSD保存時の `is-saving` 表示を、Photoshop保存用の専用アニメーションへ置き換えた。
- Photoshop保存中の残りページ数表示を、読込と同じ `現在/総数P` 表記に揃えた。
- 保存完了時は専用の完了リングとシール表示で閉じるようにし、読込/配置用の星空演出と混ざらないようにした。

### 読込アニメーション / タスク表示

- PDF/PSD読込中の左上説明表示を整理し、右下に全体タスクとパーセンテージを並べる表示へ変更した。
- タスク完了後はグレーアウトし、進行中のタスクだけが光るようにした。
- PDF/画像見本読込とPSD読込で、ファイル読込、解析、表示準備の段階が見えるようにした。
- 写植用ファイル選択ダイアログで見本を選択しただけの場合は、ページ数確認用の裏読み込みとして扱い、中央の読込アニメーションを表示しないようにした。

### 星座描画

- 星座描画を処理進捗から切り離し、一定タイミングでランダムな位置に描画され続けるようにした。
- 星座同士が重ならないよう、既存の星座位置との距離を見て配置候補を選ぶようにした。
- 読込中と自動配置中のどちらでも、星座が処理のカクつきに引きずられにくいようにした。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.1.0` に更新。

---

## v2.0.9: OPUSプロジェクト保存 / 読込アニメーション調整 / PSD保存復帰

この版では、Photoshop保存中心だった保存導線を OPUS プロジェクト保存へ寄せつつ、PSD保存もショートカット付きで復帰した。あわせて、プロジェクト復帰用データを `Script_Output` に集約し、ホーム画面からのプロジェクト読込、読込完了アニメーション、スプラッシュ表示を調整している。

### プロジェクト保存 / 読込

- `.opus` 拡張子のプロジェクト保存・読込に対応した。
- プロジェクト保存時に、作業PSD、テキストファイル、見本のコピーを `Script_Output` 配下へ保存し、そこから再度内容を復帰して開けるようにした。
- ホーム画面の「プロジェクトを開く」から読み込んだあと、作業画面へ遷移するようにした。
- 「プロジェクトを開く」のファイルダイアログ初期位置を `Script_Output` にした。
- 「プロジェクトを開きました」など右上から出るトースト通知を不要として削除した。

### 保存導線

- `Ctrl+S` とツールバーの既定保存を、プロジェクト保存に変更した。
- PSD保存を復帰し、`Ctrl+Shift+S` で実行できるようにした。
- PSD保存ボタンは星型の Photoshop 風 `Ps` アイコンに変更し、台割マネージャーの Photoshop ボタンに近い文字サイズ・フォントへ調整した。

### 読込 / 完了アニメーション

- プロジェクト読込時の進捗に新しい OPUS 星空アニメーションを使うようにした。
- 読込完了時にも、上下に星空が開くアニメーションを適用した。
- 完了時アニメーションの重さを抑えるため、閉じ演出中は背景雲・星座・流星などのアニメーションを停止し、上下カーテンを transform 中心の描画に整理した。
- 開いた瞬間に背景の雲が残らないよう、閉じ演出中の背景要素を即時非表示にした。
- 写植読込が 100% 付近で待機する場合、左上の処理内容を「最終調整中」と表示するようにした。
- 旧成功チェックアニメーションなど、使わなくなった完了演出コードを削除した。

### スプラッシュ

- 起動時スプラッシュウインドウを約30%拡大した。
- スプラッシュ内のアイコンとテキストサイズを大きくし、ロゴ表示アニメーションを復帰した。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.9` に更新。

---

## v2.0.8: フローティングパネル安定化 / OPUS進捗アニメーション刷新 / 読込安全性強化

この版では、PSD上のテキスト選択時に出るフローティングパネルの挙動、画像スキャン・自動配置中のOPUS進捗アニメーション、PSD読込時の安全ガードを中心に調整した。既存の編集ロジックを壊さないよう、表示位置・表示状態・進捗演出の制御を局所的に整理している。

### フローティングパネル

- テキスト選択時のルビ / ふりがなパネルがサイドバーへ被る場合、空き側へ逃がすようにした。
- パネルが選択テキストから離れすぎないよう、配置計算を調整した。
- 選択中テキストを切り替えたとき、前のテキストのフローティングパレット表示を引き継がず、対象テキストごとに表示を切り替えるようにした。
- テキスト切替時にフローティングパレットが一瞬左上へ飛ぶ挙動を抑えた。
- スタイルパレット / フォント検索のフローティング表示中に、サイドバー側のスタイルパレット / フォント検索項目が消える問題を修正した。
- `Ctrl+Shift` でテキストプロパティ表示を表示 / 非表示できる挙動を復旧した。

### 読込 / PSD安全性

- モノクロ2階調のPSDは警告ダイアログを出し、読み込まないようにした。
- 環境設定のショートカット一覧を最新の操作に合わせて更新した。

### OPUS進捗アニメーション

- 画像スキャンと自動配置の処理中表示を、`OPUS_constellation_growing.html` ベースの天の川・星座アニメーションへ置き換えた。
- アニメーションはヘッダー以外の画面全体に広がるようにし、下部の旧フローティングバー表示を廃止した。
- 読込フェーズが変わっても同じOPUSアニメーションを再生成せず、天の川・星座・タイマーを維持したまま進捗だけ次フェーズへ進めるようにした。
- 確定進捗がしばらく来ない待機時間でも天の川と星座が止まって見えないよう、ゆっくり進み続けるアイドル進捗と星座パルスを追加した。
- 完了時に `86%` 付近から `100%` へ即ジャンプしないよう、最後の伸びをイージングで埋めてから閉じるようにした。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.8` に更新。

---

## v2.0.7: テキストプロパティ編集 / フチなし点線アイコン / フォント表示名調整

この版では、PSD 上の選択レイヤーに出るテキストプロパティ表示を、その場で編集へ入れる入口として強化した。フォント名・サイズ・フチ表示をクリック可能にし、サイドパネル内の既存 UI をフローティングパネルとして呼び出して変更できるようにしている。

### テキストプロパティ編集

- 選択レイヤー上のフォント名表示をクリックすると、フォント選択パネルをその位置にフローティング表示するようにした。パネル内ではスタイルパレットを初期タブにし、従来のフォント検索へも切替可能。
- 選択レイヤー上のサイズ表示をクリックすると、サイズ編集パネルをフローティング表示するようにした。
- 選択レイヤー上のフチ表示をクリックすると、フチ編集パネルをフローティング表示し、白フチ・黒フチ・フチなし・太さをその場で変更できるようにした。
- フローティング中のフォント / サイズ / フチパネルは、外側クリック・リサイズ・スクロールに追従して閉じる / 再配置する。in-place 編集中も、これらのパネル操作では編集状態を維持する。

### フチ表示

- フチなしの表示を、サイドバーのフチボタンとレイヤー上のプロパティ表示の両方で「白い点線円のみ」に統一した。赤い斜線と斜線背景は廃止。
- フチなしでもレイヤー上にフチアイコンを表示し、そこからフチ選択パネルを開けるようにした。

### フォント表示

- `getFontDisplayName()` がフォントの alias から日本語名を優先して拾うようにし、`DFGMaruGothic-Md` は「ＤＦ中丸ゴシック体」と表示する fallback を追加。
- スタイル編集内のフォントソースタブは、スタイルパレットを先頭・初期表示に変更した。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.7` に更新。

---

## v2.0.6: OPUS UI調整 / フォント帳固定ヘッダー / 自動配置安定化

この版では、OPUSのデフォルトテーマ色、ホーム開始フロー、フォント帳、写植見本表示、複数選択時のサイズ調整、キーボード操作の細かな不具合をまとめて修正した。

### テーマ / ダイアログ

- OPUSのデフォルトテーマカラーをティールグリーンに変更し、設定画面の表記もカタカナの「ティールグリーン」に統一。
- 位置調整選択ダイアログを、写植用ファイル選択ダイアログと同じ暗さ・同系統のサイズ感に調整。
- 位置調整カードの高さは従来の高さへ戻し、`scan-adjust-choice-list` の上下中央に配置されるようにした。
- 読み込みダイアログの背景も同じ暗さに揃えた。

### 写植開始フロー / 見本表示

- ホームの写植開始フローで、写植見本の選択時・非表示ページ変更時に見本を先に読み込むようにし、開始ボタン押下時に同じ見本を再ロードしないようにした。
- 右サイドバーを閉じて再度開いた際、写植見本の表示位置が古いレイアウト幅に引っ張られてずれないよう、サイドバー開閉前の表示中心を保持してレイアウト更新を再実行するようにした。

### フォント帳

- PDF側フォント帳の `pdf-font-book-toolbar` 直下に、サンプル文字入力欄・再読み込み・カテゴリボタンを移動。
- `font-book-source-title` と `font-book-search` を2列に並べ、タイトル未選択時は検索欄が全幅になるよう調整。
- PDF側の `font-book-list-header` をスクロール範囲外へ出し、`pdf-font-book-controls` と同じ幅で上部固定表示にした。

### テキスト表示 / 自動配置

- 文字色を黒以外にした際に白ぼかしのフチが発光して見える問題を修正し、黒文字でも不要な白ぼかしが残らないようにした。
- 自動配置後の複数選択で、マウスホイールによりサイズをそろえた際、最大値または最低値だったテキストの色付き状態が戻らない問題を修正。
- 何も選択していない状態で通常の上下矢印キーを押しても、レイヤー選択送りが始まらず、原稿テキストが勝手に選択状態にならないようにした。選択中レイヤーの移動と `Alt+↑/↓` の原稿ブロック送りは維持。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.6` に更新。

---

## v2.0.5: 部分文字スタイル編集 / 原稿スタイル目印 / 検索置換 / フォント検索整理

この版では、PSD 上で選択した一部文字だけにフォント・サイズ・ルビを適用する編集体験を強化し、原稿テキストとエディタ表示にもスタイル変更箇所が見えるようにした。あわせて検索置換、フォント検索欄の入力しやすさ、選択状態保持、不要なフォント使用チップの削除を行った。

### PSD 直接編集

- in-place 編集中の文字選択範囲に対して、フォント・サイズ・ルビを部分適用できる経路を強化した。
- 部分サイズ変更時は編集ボックスの中心を維持したまま bbox を再計算し、DOM プレビューと PSD オーバーレイを即時更新するようにした。
- 文字選択中にサイドパネルの「原稿テキスト / スタイル編集」タブを切り替えても、選択範囲が失われないように退避・復元する処理を追加した。
- フォント/サイズ変更後も CSS Highlight による選択表示を維持し、入力欄フォーカス移動で選択が見失われる問題を抑えた。

### 原稿テキスト / エディタ

- `text-style-markers.js` を追加し、部分フォント・部分サイズの上書きがある文字範囲を原稿テキストパネルとエディタ表示に反映できるようにした。
- エディタ側でルビ記法を表示用テキストに変換しつつ、選択範囲を元テキスト位置へ戻せるマッピングを持たせた。
- エディタツールバーにルビボタンを追加し、選択文字列へルビ記法を挿入できるようにした。
- 原稿テキストのインライン編集・削除・更新後も、配置済みレイヤーとの対応やスタイル目印が崩れにくいように再描画処理を調整した。

### 検索置換 / 一括変換

- `find-change.js` を追加し、サイドツールバーから検索置換 / 全変換モーダルを開けるようにした。
- 置換時に `charSizes` / `charFonts` などの文字単位マップを置換後テキストへ追従させ、部分スタイルが大きくずれないようにした。
- 既存レイヤーと新規レイヤーの両方に対して、検索一致と置換を履歴付きで適用できるようにした。

### フォント / スタイル編集 UI

- `edit-font` にフォーカスしたとき、表示中のフォント名を一旦空にしてすぐ検索文字を入力できるようにした。未入力のまま外れた場合は元の表示名を戻し、`dataset.ps` は保持する。
- `font-usage-chip` と `font-usage-summary` を削除し、不要になったフォント使用チップ表示と CSS を撤去した。
- 複数サイズが混在する選択ではサイズ欄に混在値を表示し、単一サイズの選択では現在値へ同期するようにした。
- Ctrl クリックで選択中レイヤーのバッジ/枠を隠し、複数選択時には一時的に装飾を再表示できるようにした。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.5` に更新。

---

## v2.0.4: テキスト操作 / モーダル暗転 / プログレス発光 / スキャンエンジン警告

この版では、テキスト編集ショートカット、ホーム画面のスキャンエンジン未インストール警告、モーダル表示、プログレスバー表示を中心に調整した。

### テキスト編集

- テキスト選択中に `Ctrl+ArrowUp` / `Ctrl+ArrowDown` でポイントサイズを変更できるようにした。
- 同ショートカットは `Shift` 併用で大きめのステップ変更に対応。
- `Ctrl+Shift` によるテキスト情報表示/非表示のトースト通知を削除。
- 方向キーでテキストレイヤーを移動したあと、少しディレイしてからテキストプロパティを再表示するようにし、連打時のチラつきを抑制。
- アプリ内の静的テキストにテキストカーソルが入らないようにし、入力欄や contenteditable だけ選択可能にした。

### ホーム / スキャンエンジン

- 画像スキャンエンジンが未インストールの場合、ホーム画面に警告バーを表示するようにした。
- 未インストール時はホームの「書き起こし」「写植」カードをグレーアウトし、START ボタンを無効化。
- 警告バー内のインストールボタンから既存のスキャンエンジンインストール画面を開けるようにした。
- 警告バーは赤基調にし、インストールボタンの hover は背景・枠線・文字を青系に変更。
- テーマカラーに以下を追加。
  - ホーム画面のボカシ: `#3F8278`
  - アイコン hover / accent: `#5A9B91`

### ダイアログ

- 写植用ファイル選択ダイアログと位置調整選択ダイアログ表示中、背景がほとんど見えない程度まで暗転するようにした。
- 位置調整選択ダイアログの面を暗くし、背後がほんの少し透ける表現に調整。

### プログレスバー

- プログレスバーの色をテーマカラーに追従させた。
- 未読込部分の白線を削除。
- プログレス表示のフォントをホーム画面カードと同じ serif 系に変更。
- プログレスバーのエッジを軽くぼかし、濃いテーマ色の塗りに変更。
- 先端に、境界がなだらかな球体状の発光を追加。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.4` に更新。

---

## v2.0.3: OPUS ホーム画面リデザイン / テーマカラー追加 / スプラッシュロゴ統一

この版では、OPUS の起動体験とホーム画面をリリース向けに再調整した。ホーム画面は提供モックに合わせて、Cinzel / Noto Serif JP を Google Fonts から読み込み、ロゴ・バージョン・カード導線・背景ボカシ・テーマカラーを統一している。

### ホーム画面

- `home-title-word` を Cinzel SemiBold 相当の `OPUS` テキスト表示にし、文字サイズと字間を大きく取ったロゴ表現へ調整。
- `home-version` を上部ロゴ直下ではなく、下半分の黒い領域の境界少し下に配置。文字サイズを小さくし、色をややグレー寄りにした。
- バージョン下の中央区切り線を短い線に変更し、バージョン表記より少し長い程度の幅へ調整。
- 背景のブルー系ボカシは表示範囲を上へ寄せ、上半分がウィンドウ上部に隠れるようにした。明度も落として、画面全体の黒背景となじませた。
- ホームカードは約 1.2 倍に拡大し、カード同士の間隔・上部区切り線との余白・小さいウィンドウ時の余白を調整。
- カードの外枠とカード内区切り線を細くし、START ボタンは白枠 / 白文字の Cinzel italic に変更。三角アイコンは小さめにして、ボタン角は少し鋭くした。

### テーマカラー

- 環境設定に「テーマカラー」タブを追加し、写植設定タブの後ろに配置。
- テーマカラーは既定のブルーに加え、アンバーとクラシックブルーを追加。
  - ホーム画面のボカシ: `#BE8B43`, `#005A9E`
  - 選択中カラー: `#D4A55F`, `#0078D4`
- 既定ブルーの選択中カラーは視認性を優先して `#6076E5` にした。
- 選択中リング、ボタン押下状態、各種アクセント色がテーマカラーに追従するようにした。

### スプラッシュ

- 起動スプラッシュの `opus_logo.png` 表示を、ホーム画面と同じ `home-title-word` の `OPUS` テキスト表示に置き換え。
- スプラッシュ側にも Google Fonts の Cinzel を読み込み、ホーム画面と同じ字間を反映。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.3` に更新。

---

## v2.0.2: テキスト編集同期 / 自動配置レイアウト修正 / 文字色カスタムパレット

この版は、PSD 上での直接編集、自動配置後のテキスト枠フィット、ショートカット誤発火、文字色指定まわりをまとめて修正した安定化リリース。文字色は MojiQ のカスタムカラーパレットに近い操作感で、固定色と任意 HEX 色をテキストへ反映できるようにした。

### 選択表示 / ショートカット

- `Ctrl+Shift` でテキスト選択時のテキストプロパティ・枠線・サイズバッジ・回転ハンドルの表示/非表示を切り替えられるようにした。
- `Ctrl+Windows+S` など Meta/Windows キーを含むショートカットで、テキストプロパティ表示切替が誤発火しないよう修正。
- テキスト選択表示を隠した状態でも内部選択は保持し、再表示時に選択状態が復元されるようにした。

### PSD テキスト編集同期

- PSD 上の in-place テキスト編集内容を、原稿テキストとテキストエディタへ同期するよう修正。
- PSD 側で文字を削除・変更した場合、配置元 TXT 段落と右側エディタ表示に古い文字が残らないようにした。
- 他テキストから一部をカットした直後、カット元が選択されたまま別テキストをクリックすると位置が入れ替わる問題を、クリック/ドラッグ判定の閾値追加で防止。

### 自動配置 / テキスト枠フィット

- 自動配置後の複数行テキストで行が崩れる問題を修正し、改行を CSS 側で勝手に折り返さないよう `white-space: pre` ベースに変更。
- テキスト枠の下・左右に残っていた余白を削減し、実描画テキストに近い矩形へ auto-fit する処理を追加。
- 1 行テキストにも複数行と同じ bbox 計算・安全幅計算を適用し、1 行だけ幅や位置がずれるケースを抑制。
- 回転中のテキストは auto-fit 対象から外し、回転操作で文字が別行へ流れる問題を修正。
- マウスで直接移動した後に配置が少しずれる問題を、auto-fit が left/top を上書きしない方式に変更して解消。
- 自動配置でフォント自動切替により色付けされたテキストは、フォント変更だけでなく、ダブルクリック編集モード移行・サイズ調整でも黒へ戻るようにした。

### 句読点 / カッコのツメ

- 句読点ツメの対象に `「」` と `〝〟` を追加し、句読点と同様に 50% ツメを適用できるようにした。
- 開きカッコ系は詰め方向を調整し、次の文字へかかって見える問題を修正。
- フロントのプレビュー、自動配置スキャン、Photoshop 書き戻し用 JSX の対象文字セットを揃えた。

### 文字色パレット

- 文字色パレットを右パネルの `editor-tabs-section` 内に追加し、旧サイドツールバー下部の `fill-color-picker` は削除。
- パレットの並びを `黒 / 白 / 赤 / 青 / カスタム` に整理し、「文字色をそのまま」ボタンは UI から削除。
- カスタム文字色は未選択時に背景なし + 点線枠で表示し、選択後は選択色の塗り + 実線枠に変わるようにした。
- `input[type=color]` による任意 HEX 色を選択中レイヤー・新規配置テキスト・Photoshop 保存へ反映できるようにした。
- PSD 読み込み時に白黒以外の塗り色も `#rrggbb` として保持し、プレビューと再保存で維持できるようにした。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.2` に更新。

---

## v2.0.1: 写植 UI polish / ルビ削除修正 / フォント帳 PDF 表示 / 自動配置進捗安定化

この版は、v2.0.0 の配布後に実機フィードバックを受けて行った UI・編集挙動・自動配置フローの修正版。既存インストールが v2.0.0 を取得済みでも updater が更新として認識できるよう、バージョンは `2.0.1` に上げる。

### 位置調整ダイアログ

- 位置調整選択ダイアログを写植選択ダイアログ相当のサイズへ広げ、`ai-adjust-choice-option` を `home-typeset-row` と同系統のグラデーション付き立体カードに統一。
- 位置調整カードの点線装飾を削除し、hover 時は余白図のグレー面を変えず、カード枠線のみ青く変化するよう調整。
- 図中の PSD 白面には下方向から濃い薄暗いグラデーションを付け、カード背景と分離して見えるようにした。
- 「位置調整なし」の図を、テキストアイコン SVG → 矢印 → PSD のイラストへ変更。ダークモード時は T / 矢印を白にし、hover で青く変わらないよう固定。
- 「位置調整なし」「位置調整2」「重ね調整」の図をカード中央へ寄せ、矢印先端の切れ・PSD への被り・T との距離を調整。

### PDF / PSD 表示とズーム

- `Ctrl+0` は 100% 固定ではなく、画面フィット相当の 110% 表示へ戻す挙動に変更。
- PDF 側で `Ctrl+0` した場合も左上基準へ戻らず、表示中ページが画面にフィットするよう viewport リセット処理を調整。
- `pdf-stage-label-bar` に本アイコンボタンを追加し、PDF 表示エリア内の `spreads-pdf-area` にフォント帳を表示できるようにした。
- フォント帳をサイドバータブから削除し、PDF エリア側の切替 UI に一本化。
- PDF/フォント帳読み込み完了時の「読み込みました」通知は出さないようにした。

### フォント / テキスト編集 UI

- フォントお気に入り登録はフォント情報だけを保存し、同一フォントが既に登録済みの場合は重複追加しない。
- フォントお気に入り登録時の確認ダイアログを廃止し、即時登録にした。
- お気に入り削除ボタンは hover 時に削除アイコンが赤く見えるように調整。
- 太字ボタンに加えて斜体ボタンを新設し、両方を `font-weight-selector` の下へ配置。
- 複数サイズのテキストを選択してホイールでサイズ変更した場合、まず最大値/最小値へ揃え、その後に全体を同じ差分で増減する仕様へ変更。
- テキスト選択時の使用フォント情報を `edit-font` の入力表示へ反映し、不要になった `font-usage-label` を削除。

### ルビ

- ルビ削除ボタン押下後に選択状態へ戻ってもルビが残る問題を修正。
- 二行目以降の文字に付けたルビを削除した際、行間がルビ付きのまま残らないよう `lineLeadings` を再計算・クリアする処理を調整。
- ルビを持たない文字列を選択した場合は、以前選択したルビ設定を引きずらず、ルビ種別を「自動」へ戻すようにした。

### ヘッダー / ツール操作

- ヘッダー内のページリンクボタンと表示/非表示ボタンを削除し、リンク状態をデフォルトにした。
- `scan-actions-row` の表示ボタン群を削除し、内部機能だけは残す形にした。
- 方向キーでテキスト位置を移動した場合もテキストプロパティを非表示にするようにした。
- 常時表示の回転ハンドルを削除し、`Ctrl+T` 選択時だけ回転ハンドルを表示して回転できる方式へ変更。テキスト以外をクリックすると回転モードをキャンセルする。

### 自動配置 / 進捗演出

- 画像スキャン、PSD 読み込み、見本読み込み、自動配置、位置調整が別々に進捗モーダルを閉じ開きしていたため、ホーム一括フローでは一連の流れが完了するまで同じ読み込み演出を保持するようにした。
- `runScanExtract` に `keepProgressOpen` を追加し、自動配置から呼ばれた画像スキャン成功時はモーダルを閉じずに自動配置へ引き継ぐ。
- `loadReferenceFiles` / `loadPsdFilesByPaths` にも `keepProgressOpen` を追加し、ホーム写植フローでは前段読み込みでも背景ボカシを閉じない。
- `runAutoPlace` は同時・連続呼び出し時に同じ Promise を返すガードへ変更し、自動配置が二重に走ってアニメーションが二回開閉する問題を抑止。
- スキャン完了後の自動配置起動は、ボタンクリック経由ではなく `runAutoPlace()` の直接呼び出しに変更。
- 失敗・キャンセル・配置対象なしの分岐では、保持している進捗モーダルを確実に閉じるよう補正。

### Version

`package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `2.0.1` に更新。

---

## v1.4.1: 写植一気通貫フロー / 位置調整最適化 / 非表示見本除外

### 写植開始フロー

- ホームの写植用ファイル選択で見本 / PSD / テキストを選んだ後、開始時に位置調整選択ダイアログを表示する流れに変更。
- 見本カード内の位置調整ボタンは削除し、開始操作から一気通貫で画像スキャン、自動配置、選択した位置調整まで進むように整理。
- 位置調整選択ダイアログはカード式の横並びにし、「位置調整なし」を追加。
- 見本がPSDページ数より多い場合は、PSD枚数分だけOCRと自動配置を実行する仕様に変更。見本不足時のみ警告する。

### 非表示見本とOCR

- 非表示にした見本画像 / PDFページは、自動配置時の表示・ページ計算だけでなくOCR前処理からも除外。
- Rust側のAI OCR処理に `excluded_pages` を渡し、画像コピーやPDF展開の段階で非表示ページをスキップ。
- 非表示選択はShift押下で範囲選択できるように変更。
- すべての見本が非表示になっている場合は「すべて非表示になっています」と警告し、再選択させる。

### 位置調整

- 自動配置時に位置調整1 / 2の座標計算を先に行い、配置後の再走査を省略して処理時間を短縮。
- 位置調整1はフロント側のサイズ差計算を使い、不要なRust画像デコードを避けるように変更。
- PDF見本の位置調整2はアプリ内PDF描画を優先し、Rust側でPDFを毎回開き直す経路を削減。
- 位置調整のプログレスバーは実際の完了ページ数に同期するように修正。

### UI / PSD / ルビ / 保存

- ルビパネルは文字にカーソルが入っている場合のみ表示し、ウィンドウ外へはみ出さないよう位置計算を修正。
- サイドバーのルビ編集UIを削除。
- PSD表示のフィットと中央位置を修正し、ウィンドウサイズ変更時の上寄りや収まり不良を改善。
- 複数テキスト選択時、マウスホイールでポイント数をまとめて変更できるように対応。ポイント数が混在する場合はスクロール方向に応じて最大 / 最小へ揃えてから変更する。
- テキスト上以外でスクロールしても、選択中テキストのサイズ変更が効くようにした。
- Photoshop保存処理中はPhotoshopウィンドウを表示せず、処理状況をOPUS上で表示。
- 処理中プログレスバーを細くし、アイコン直下へ配置。
- 「テキスト編集」タブを「スタイル編集」に変更。
- ヘッダーの先頭白紙ページ除外ボタンを削除し、非表示設定はアイコンのみのボタンから開閉できるように整理。

### 整理とバージョン

- 確度の高いデッドコードを削除。
- 起動時に過去バージョンの更新ダイアログが出ないよう、バージョン表示・比較まわりを整理。
- `package.json` / `package-lock.json` / `src-tauri/Cargo.toml` / `src-tauri/Cargo.lock` / `src-tauri/tauri.conf.json` を `1.4.1` に更新。

## 2026-05-21 変更メモ: 表示フィット / ルビUI / フォントお気に入り / 文字化け再発防止

このセッションでは、PDF/PSD 並列表示のズーム・フィットずれ、閲覧モードとテキストエディタモード切替時の表示位置、テキスト編集まわりの UI、写植開始フロー、フォント検索・お気に入り、ルビ編集、文字化け修復と再発防止を中心に修正した。

### PDF / PSD 表示・ズーム・閲覧モード

- PSD / PDF の表示倍率は、実際の初期フィット倍率をアプリ上の `100%` として扱うようにした。従来の見た目 110% 相当を基準倍率にし、ズーム表示は基準倍率からの相対値として計算する。
- `Ctrl+0` は単に倍率を `100%` に戻すのではなく、表示領域に画像全体が収まるフィット表示へ戻す処理に統一した。
- PSD 側でズーム後に `Ctrl+0` したとき右下が見切れる問題を修正し、フィット時はスクロール位置もリセットしてページ全体が表示領域に収まるようにした。
- PDF 側も PSD と同じ考え方でフィット・ズーム位置・スクロール位置を扱うようにし、テキストエディタ / 閲覧モードから戻ったときに画像がずれる問題を修正した。
- 閲覧モードは通常表示のズーム位置を引き継がない。閲覧モード内では独立して表示し、戻った後は元の通常表示側の状態を破壊しない。
- 閲覧モードでも `Alt + wheel` ズームに対応した。
- 閲覧モードのショートカットは表示 / 非表示ともに `Esc` 固定にした。
- 閲覧モード後に `Ctrl+0` を押しても PDF / PSD がフィットからずれないよう、閲覧モード終了後のレイアウト計算とフィット再計算の順序を整理した。
- テキストエディタの「見本」表示は通常ビューのズーム位置を反映せず、常に画像にフィットして表示するようにした。

### テキスト編集・選択表示・ルビ

- Alt 複製したテキストが原稿テキストとテキストエディタ側に追加されない問題を修正した。
- 方向キーの移動距離の既定値を `6` に変更した。
- テキスト選択時に表示するフォント名、ポイント数、フチ情報、回転ハンドルは、ドラッグ移動中は非表示にするようにした。
- フチを付けていない場合は、選択中メッセージに「フチ無し」アイコンを表示しないようにした。
- `ruby-line-hint` は表示しないように削除した。
- 個別テキスト選択時、テキスト右側にルビ用の `editor-tab-panel` を表示し、選択テキストに対してルビを付けられるようにした。
- ルビパネルが選択中テキストのメッセージ表示と重ならないよう、表示位置を調整した。
- ルビ適用時に壊れていた `startLine` / `endLine` 算出を復旧し、選択範囲から行番号を計算してルビ情報へ反映するようにした。

### 写植開始・自動配置・見本管理

- 写植スタートボタンで、ファイル / フォルダ選択のリンクパスを貼り付けた場合も対象へ遷移できるように修正した。
- 写植スタートボタンが押せなくなっていた問題を修正した。
- 画像スキャンから自動配置するとき、テキストデータを入れてテキスト照合する場合は、セリフのないページをスキップするようにした。
- ヘッダーの「画像」「PSD」「テキスト」読み込みボタンを削除した。
- 非表示にする見本選択ダイアログで、見本カードが表示済みなのに「見本を読み込み中...」が残る問題を修正した。
- 非表示見本選択ダイアログ右上の × ボタンを削除した。
- 非表示見本選択ダイアログは「写植用ファイルを選択」と同等の固定サイズにし、複数ファイルの一覧は `reference-hidden-body` 内でスクロール表示するようにした。

### フォント検索・お気に入り

- テキスト編集のフォント検索下にお気に入り機能を追加し、現在のフォント、ポイント数、拡大率、フチ設定などのスタイルを保存できるようにした。
- お気に入り登録はブラウザ標準ダイアログではなく、アプリ内のカスタムダイアログに変更した。
- お気に入り一覧はスクロールボックス内に表示し、項目が増えてもウインドウ外へはみ出さないようにした。
- お気に入りスクロールボックスの高さを広げ、一覧を見やすくした。
- お気に入り追加ボタンは `+` に加えて「追加」の文言を表示するようにした。
- お気に入り削除ボタンは × ではなくゴミ箱アイコンへ変更した。
- フォント検索は Photoshop のように和名でも検索に引っかかるようにし、PostScript 名 / 英字名 / 日本語名を照合対象にした。
- フォント候補の見本表示も日本語フォント名を表示できるようにした。

### ダイアログ・UI 表示

- 見本選択やお気に入り登録など、標準ダイアログに寄っていた箇所をアプリ内カスタムダイアログへ寄せ、既存 UI と見た目を合わせた。
- 一覧型 UI は固定領域 + 内部スクロールにし、ウインドウサイズ外へ項目がはみ出さない方針に統一した。
- 表示切替やモード変更後に残るメッセージ、ロード表示、選択バッジを整理し、実際の状態と UI 表示が食い違わないようにした。

### 文字化け修復とロジック破損の調査

- `src/main.js` の文字コード操作で壊れた日本語文字列を修復した。
- PowerShell の表示上の文字化けと実ファイルの UTF-8 破損を混同しないため、必要に応じて Node.js で `JSON.stringify` した実文字列を確認する方針にした。
- 行番号ベース削除の影響で消えていたロジックを調査し、ルビ適用処理の `startLine` / `endLine` 未定義を修正した。
- `src/main.js` の残存文字化けコメントを修正した。
- 文字化け疑いパターンを `rg` と Node.js ベースの検査で確認し、ユーザー表示文字列に残っていないことを確認した。

### 再発防止

- `scripts/check-source-integrity.mjs` を追加し、ソース内の UTF-8 BOM と疑わしい文字化けコードポイントを検出するようにした。
- `eslint.config.js` を追加し、`no-undef` をエラー化した。これにより、行削除や import 破損で未定義変数が混入した場合に検出できる。
- `package.json` に `check`, `check:encoding`, `lint` を追加した。
  - `npm run check:encoding`: 文字化け / BOM 検査。
  - `npm run lint`: 未定義変数などの静的検査。
  - `npm run check`: 文字化け検査、lint、ビルドをまとめて実行。
- `src/ai-ocr.js` の `notifyDialog` import 漏れ、`src/bind/editor-pane.js` の `ensureTxtExtension` / `pickTxtSavePath` import 漏れを lint で検出し修正した。
- 今後は日本語を含むファイルに対して、PowerShell の `Set-Content` や行番号ベースの一括削除を避け、手作業の修正は `apply_patch`、機械的な検査は `npm run check` で確認する。

### 検証

- `node --check src/main.js` 成功。
- `npm run check` 成功。
- `npm run build` 成功。
- `cargo check` 成功。既存の `src-tauri/src/alignment.rs` の unused warning は残るが、今回の変更による失敗ではない。

---

## 2026-05-19 OPUS v1.30.9 変更メモ

このセッションでは、ホーム画面、写植開始フロー、フォント帳、PDF/PSD 自動配置、編集サイドバー、リリース表記を中心に OPUS 向けの統合変更を行った。

### ブランディング / ホーム画面

- アプリ内表示の `PsDesign` を `OPUS` に置換し、Tauri の `productName` / ウィンドウタイトル / デスクトップ表示名も OPUS に統一。
- ホームのロゴバーを調整し、アイコン削除、タイトルロゴ縮小、バー高さ縮小、`Ver 1.30.x` 表記をロゴバー右側に表示。
- アプリ全体の表示フォントをヒラギノ角ゴ系に変更。
- ホームカードの文言を「画像からテキストを抽出」「自動配置による写植」に変更し、カード内メッセージを中央揃え。
- ライトモード時のホームアイコンシャドウ、スタートボタンの反転色、ホバーシャドウなどを調整。

### 写植開始 / 自動配置フロー

- ホームの写植スタートから、見本・PSD・任意テキストを選択する専用ダイアログを表示するように変更。
- ダイアログはドラッグ＆ドロップとファイル選択に対応し、読込済みカードには左上バッジとファイル数を表示。
- PDF のページ数をファイル数に反映し、横長 PDF は見開きとして 1 物理ページを 2 ページ相当に換算。
- 「先頭白紙ページを除外」を見本カードに追加し、見開き原稿でも先頭ページ除外が 1 ページ分として扱われるよう補正。
- 画像読込中はカード内にプログレスバーを表示。
- 開始時に見本ページ数と PSD 数が合わない場合は警告ダイアログを表示。
- 見開き原稿の自動配置では、見開きの半分を 1 ページとして PSD と対応付ける。

### 書き起こし / OCR

- ホームの書き起こしスタートから、画像スキャン、テキスト抽出、テキストエディタモード表示まで自動で進むように変更。
- アプリ化後の画像スキャンでターミナルウィンドウが表示されないよう、バックエンド処理の起動方式を調整。
- OCR ソースパネルを `spreads-editor-area` の右側へ配置し、エディタツールバーのチェックボックスで表示切替できるようにした。

### フォント帳

- フォント帳メタ情報と「フォント帳を開く」ボタンを削除し、再読み込みボタンをサイズ切替の右側へ移動。
- G ドライブの `写植・校正用テキストログ` 配下から作品情報を選び、`fontbook.json` を読み込むフローに変更。
- 作品情報選択ダイアログは、選択中フォルダの中身を一覧表示する形式に変更し、フォルダを選択した場合は中の `fontbook.json` を自動読込。
- フォルダ / ファイルアイコンを PSD を開くダイアログと同じ SVG 系アイコンに統一。
- 上へボタンをテキストから上向き矢印アイコンへ変更。
- フォント帳カテゴリをハンバーガーメニューへ集約し、サンプル文字入力もメニュー内へ移動。
- 「使用あり」ボタンと `font-book-selected-info` を削除。
- `font-book-list` には読み込んだフォント帳フォルダの親作品名を表示。
- フォント帳画像はサイドバーに直接表示せず、選択時の画像ダイアログで表示するように変更し、ローカル画像パスは Tauri 側経由で表示できるようにした。
- 画像ダイアログの閉じる操作を右上の × から、情報欄右側の「閉じる」ボタンへ変更。

### 編集 UI / サイドバー

- サイドバーの位置調整メニューをホバー式ドロップダウンから選択ダイアログへ変更。
- 警告系ダイアログのタイトル色を赤で統一。
- ダイアログやボタンのホバー背景を既存ダイアログメニューと同じ薄いグレーに統一。
- 選択中テキストのフォント名・文字サイズ情報を、該当テキストから引出線でつなげて表示。
- 選択中テキストのサイズを環境設定の文字サイズに統一するボタンをサイドツールバーへ追加。
- ルビや文字サイズ変更、テキストエリアクリック時に選択状態が解除されないよう選択維持処理を調整。
- 自動配置で色が変更されたフォントを変更した場合は色を解除し、紫のデフォルト色表現を廃止。

### 環境設定 / 操作

- 文字サイズ変更幅に `0.25` を追加。
- 「ページ送り」を「方向キー」に文言変更し、方向キー移動距離を設定できるようにした。
- ファイル選択時の `Ctrl+A` 全選択に対応。

### リリース

- `package.json` / `package-lock.json` / `src-tauri/tauri.conf.json` のバージョンを `1.30.9` に更新。
- `CLAUDE.md` に本変更メモを追記。
...
## v1.30.5: scan engine UX / font book / viewer and text locks

### Font book

- Comic-Bridge DEMO の「フォント帳」を PsDesign に移植し、サイドパネルへ `フォント帳` タブを追加。
- `src/font-book.js` を新設し、`fontbook.json` とサンプル画像を読み込む一覧・検索・カテゴリ表示・プレビューを実装。
- フォント帳タブ押下時のフリーズを避けるため、fontbook entries 由来のフォントだけを対象にし、サンプル表示は `IntersectionObserver` で遅延読み込みする。
- Rust 側に `read_text_file` コマンドを追加し、フロントから fontbook JSON を安全に読み込めるようにした。

### Viewer mode

- 閲覧モード中もサイドバーとツールバーを表示できるように調整。
- 閲覧モードの終了ボタンを `psd-stage` 左上基準に配置し、ヘッダーと重ならないようにした。
- 閲覧モード中はヘッダーを非表示にし、PSD 表示領域を優先する。

### AI / scan engine installer

- UI 表記を `AIインストール` から `スキャンエンジンインストール` に変更し、アイコンを下向き矢印へ変更。
- インストール画面の説明文を `以下のAIモデルを...` から `以下のスキャンエンジンを...` に変更。
- インストールログ (`ai-log-viewer`) 内にも進捗バーを表示し、pip ダウンロード行が無いフェーズでも進行状況が分かるようにした。
- インストール完了時に `lastDownload` が残っていて進捗バーが途中表示になる問題を修正し、完了時は必ず 100% / `インストール完了` にする。
- アンインストール時もログ内に進捗バーを表示し、開始 / 削除中 / 完了 / 失敗を可視化する。
- アンインストール時は削除済み項目と削除失敗項目をログへ出力する。
- インストール / アンインストール中の二重実行を防止し、実行中は関連ボタンを disable する。
- 画像スキャンエンジン未インストール時は `ai-actions-row` をロックし、画像スキャン / 自動配置 / 位置調整をクリック不可にした。
- ロック表示は赤い文字 + 赤い点線で表示し、ボタン本体だけをグレーアウトすることで赤表示が grayscale に巻き込まれないようにした。

### AI action panel

- `ai-actions-row` のレイアウトを、画像スキャン / 自動配置 / 位置調整の 3 ボタン構成へ変更。
- 位置調整は hover でドロップダウンを開き、`位置調整1` / `位置調整2` / `重ね調整` を選択する UI に変更。
- パネルとドロップダウンをライトモード対応にし、ボタン高さ・文字サイズ・オレンジ色・区切り線を調整。

### Auto placement colors and selected text badge

- 中丸ゴシック自動配置時の色分けを以下へ変更:
  - 50-59% bucket 0: 紫 `#9400d3`
  - 60-69% bucket 1: 赤 `#ef4444`
  - 70-79% bucket 2: 橙 `#f97316`
  - 80-89% bucket 3: 黄 `#eab308`
  - 90-99% bucket 4: 青 `#3b82f6`
  - 100% bucket 5: 緑 `#32cd32`
- テキスト選択中のポイント数を白文字のバッジ表示に変更。

### Text/editor locks

- 原稿テキストが未生成 / 未読込 / 空のときは `テキスト編集` タブをグレーアウトしてロックする。
- 保存済みのサイドパネル状態が `editor` でも、テキストが無い場合は自動的に `原稿テキスト` タブへ戻す。
- テキストが無い場合は、サイドツールバーの全テキスト選択ボタンとレイヤーボタンもグレーアウトしてロックする。
- テキストが空になったときは、開いているレイヤードロワーを自動で閉じる。

### Version

`package.json` / `src-tauri/Cargo.toml` / `src-tauri/tauri.conf.json` を `1.30.5` に更新。

---

## v1.30.6: home screen / splash / hover interaction polish

### 起動・ホーム画面

- 起動時に `public/splash.html` のスプラッシュスクリーンを表示し、Rust 側で `splash` ウィンドウを先に開いてから `main` ウィンドウを表示する構成に変更。
- メインウィンドウは初期状態を `visible: false` にし、`close_splash` コマンドでスプラッシュ進捗を 100% まで進めた後に表示する。
- ホーム画面を追加し、ロゴ / アプリ名 / 「書き起こし」「写植」のスタートカードを表示。
- スタートボタン押下時はホーム画面をフェードアウトしてメイン画面へ遷移し、画像スキャンダイアログを開く。
- ホームへ戻る操作時も画面が奥に引くようなフェード演出を追加。
- ホーム画面はライトモードにも対応し、ロゴ領域・カード背景・文字色・シャドウをテーマごとに調整。
- ホーム画面ではヘッダーの利用不可ボタンをグレーアウトし、アプリアイコン / ハンバーガー / 環境設定 / ホーム系の扱いを整理。
- 環境設定ボタンとホームボタンをハンバーガー右隣へ移動し、最終順を `アプリアイコン / ハンバーガー / 環境設定 / ホーム` に変更。

### 画像スキャン導線

- `openAiOcrDialog({ force })` を追加し、ホーム画面のスタートボタンから既存テキスト破棄確認、ファイル選択、見本表示、OCR 実行までを共通処理で呼べるようにした。
- PSD / PDF / テキスト未選択状態でも、ホーム開始導線から画像スキャンを自然に開始できるようにした。

### UI アニメーション・ホバー

- タブ選択時の背景色変化にアニメーションを追加し、サイドバータブ、フォント帳ボタン、使用あり / SML などの切替ボタンもふわっと変化するように調整。
- 通常ボタンのホバーはアイコンや文字色が青へ変化する方針に整理。
- 青系の主要ボタンは通常時を青枠、ホバー時を青背景 + 白文字へ統一。
- ホーム画面のスタートボタンは青枠スタイルにし、ホバー時に青背景へフェード。ライトモードでは通常時の文字を黒に調整。
- キャンセル系ボタンはホバー時に赤背景 + 白文字へ統一。共通ダイアログ、ファイル選択、位置合わせ、校正ブラウザ、危険操作ボタンを対象にした。
- ホバー時に文字が背景色へ同化していた「戻る」などのボタンを修正。

### AI action panel

- `ai-actions-row` のボタン枠線を細くし、ホバー時は各ボタン色で背景を塗って白文字になるように変更。
  - 画像スキャン: 青背景 + 白文字
  - 自動配置: 緑背景 + 白文字
  - 位置調整: オレンジ背景 + 白文字
- ライトモードでも同じホバー挙動になるよう上書きスタイルを追加。
- 位置調整ボタンの枠線も 1px に揃えた。
...
## v1.30.10: 写植フロー改善 / ルビ・白フチ保存最適化

### ファイル選択・位置調整

- 写植用ファイル選択ダイアログの見本 / PSD / テキストカードのドラッグ操作を改善し、PSDカードへの直接ドロップとフォルダ単位ドロップに対応。
- ドラッグ中のカード枠は点線から青い実線へ切り替わるようにし、ドロップ可能状態を視覚的に分かりやすくした。
- 位置調整ダイアログに図入りの選択肢を追加し、ホバー時は背景を塗り替えず枠線だけで選択状態を示すように変更。
- 位置調整後の PSD 表示倍率を表示領域に合わせた上で約 10% 拡大し、その状態をアプリ上の 100% 表示として扱うよう調整。
- 見本画像の非表示選択ダイアログを追加し、選択した見本ページは非表示かつページ計算から除外されるようにした。先頭白紙ページ除外もこのダイアログへ移動。

### テキスト編集・フォント

- テキスト編集後にページ移動すると編集前へ戻る問題を修正。
- AボックスからカットしたテキストをBボックスに貼り付けた後、Aを削除するとBのテキストまで消える問題を修正。
- 一文内に複数フォントがある場合に備え、選択中テキストで使用中のフォント情報を扱えるようにし、既存レイヤーの per-char font を編集表示へ反映。
- 基本フォントの初期値を `F910コミックW4-IPA Regular` に変更。
- フォント帳のカテゴリ選択後に他カテゴリボタンが消える問題を修正。
- フォント選択時のバッジ表示を整理し、選択中テキストのフォント名、ポイント数、現在のフチ状態をカード上に表示。
- ポイント数とフチ状態は横並びにし、フチバッジは現在選択中の状態だけを表示。
- 「選択中のテキストサイズを統一」ボタンが単一テキスト選択時にも効くように修正。

### 自動配置・OCR / TXT

- 自動配置後にテキスト色を打ち換えた場合、手入力編集後は自動カラー扱いから外れるよう調整。
- OCRチェックが入った状態で同期 / 非同期モードを切り替えた際、`ocr-source-panel` がテキストエディタモード以外に残る問題を修正。
- TXTが `1Page` から始まっていない場合でも、読み込んだPSDの番号が若い順にテキストを流し込むように変更。
- 参考実装のルビ機能を取り込み、TXT内のルビ注記、OCR/TXT差分表示、ルビ行間、PSD保存時のルビ出力を一連のフローで扱えるようにした。

### PSD保存・白フチ・ルビ

- Photoshop保存時の白フチは、複製下敷きレイヤーではなくテキスト / グループ自体の境界線効果として保存する方針に統一。
- 「―」「～」など連続記号に白フチを付けた際、文字ごとのフチが重なって見える問題を軽減するため、プレビュー側は塗り文字とフチ下敷きを分離。
- 白フチ付きテキストへカーソルが入らない問題を避けるため、編集対象からフチプレビュー用 underlay を除外。
- 白フチの角が硬く出すぎないよう、プレビュー側の stroke join / cap とぼかしを調整。
- 保存時のルビ + 白フチ処理を最適化。ルビ生成前に白フチを先に適用して Photoshop bounds が変わる経路を削除し、文字装飾・ルビ・回転処理後に一度だけフチを確定する。
- ルビあり + 白フチありの新規レイヤーでは、親文字とルビをサブグループ化し、そのグループ側へ境界線効果を付けることで、親文字とルビをまとめて自然な白フチにする。
- 旧式の `__PsDesignStroke__` 下敷きレイヤー削除 / 複製ロジックを撤去。

### UI / 操作性

- 作品情報選択ダイアログの上へボタンを調整し、アイコンサイズと枠線がバー内に収まるように修正。
- Vツール選択中、方向キーで選択中テキストボックスを微調整できるようにした。Shift + 方向キーは大きめに移動。
- スプラッシュ画面のロゴを中央配置に調整。
- ヘッダー右側のバージョン表示をハードコードからアプリバージョン参照へ変更し、バージョン更新時に自動反映されるようにした。
