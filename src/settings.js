...
export const DEFAULT_SETTINGS = {
  ...
  defaults: {
    ...
    showBadge: true,
    // ダブルクリック編集中だけテキストを 2 倍表示する（保存される文字サイズは変更しない）。
    // scope: render-all
    inPlaceEditZoomEnabled: true,
    // V ツールでテキストフレームに一定時間ホバーしたとき、自動で選択するか。
    // scope: interaction-only
...
function migrate(old) {
  ...
  if (old.fileDialogMode === "opus" || old.fileDialogMode === "native") {
    out.fileDialogMode = old.fileDialogMode;
  }
  if (typeof old.arrowKeyMoveDistance === "number" && Number.isFinite(old.arrowKeyMoveDistance)) {
    ...
    out.arrowKeyMoveDistance = sourceVersion < 15 && migrated === 1
      ? DEFAULT_SETTINGS.arrowKeyMoveDistance
      : migrated;
  }
  if (old.defaults && typeof old.defaults === "object") {
    ...
    if (typeof d.textSizeStep === "number" && (d.textSizeStep === 0.1 || d.textSizeStep === 0.25 || d.textSizeStep === 0.5 || d.textSizeStep === 1)) {
    ...
    if (typeof d.inPlaceEditZoomEnabled === "boolean") {
      out.defaults.inPlaceEditZoomEnabled = d.inPlaceEditZoomEnabled;
    }
    if (typeof d.hoverSelectEnabled === "boolean") {
      out.defaults.hoverSelectEnabled = d.hoverSelectEnabled;
    }
    if (typeof d.textFitEnabled === "boolean") {
      out.defaults.textFitEnabled = d.textFitEnabled;
    }
...
export function getFileDialogMode() {
  if (!settings) load();
  return settings.fileDialogMode === "native" ? "native" : "opus";
}

export function setFileDialogMode(mode) {
  if (!settings) load();
  const next = mode === "native" ? "native" : "opus";
  if (settings.fileDialogMode === next) return;
  settings.fileDialogMode = next;
  save();
}
